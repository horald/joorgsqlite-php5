...
if ($roomtyp=="ETAGEN") {
  ...
  $sqlanz="SELECT count(*) AS anz FROM tblorte WHERE fldid_etage=".$etagenid." AND fldview='J' AND fldo01typ='ZIMMER'";
  //$results = $db->query($sql);
  $anzbez="0 Moebel";
...
if ($roomtyp=="MOEBEL") {
  ...
  $sql="SELECT * FROM tblorte WHERE fldid_moebel=".$index." AND fldview='J' AND fldo01typ='FAECHER'";
} 
...
while ($row = $results->fetchArray()) {
  ...
  if ($roomtyp=="ETAGEN") {
  	 $anzbez="0 Möbel";
    ImageString ($bild, 5, $row['fldxkoor']+5, $row['fldykoor']+20, $anzbez, $schwarz);
  }  
}
